    ...
    public class FirestoreRepository<T> : IRepository<T> where T : class
    {
        ...
        public virtual async Task<bool> UpdateFieldsAsync(string id, Dictionary<string, object> updates)
        {
            try
            {
                ...
                {
                    return false;
                Console.WriteLine($"🔥 [Firestore Hit] UPDATE collection: {_collectionName}, ID: {id}");
                var docRef = _db.Collection(_collectionName).Document(id);
                var snapshot = await docRef.GetSnapshotAsync();
        ...
        public virtual async Task<bool> UpdateFieldsAsync(string id, Dictionary<string, object> updates)
        {
            try
            {
                Console.WriteLine($"🔥 [Firestore Hit] UPDATE FIELDS collection: {_collectionName}, ID: {id}");
                var docRef = _db.Collection(_collectionName).Document(id);
                var snapshot = await docRef.GetSnapshotAsync();
        ...
        public virtual async Task<bool> DeleteAsync(string id)
        {
            try
            {
                Console.WriteLine($"🔥 [Firestore Hit] DELETE from collection: {_collectionName}, ID: {id}");
                var docRef = _db.Collection(_collectionName).Document(id);
                var snapshot = await docRef.GetSnapshotAsync();
